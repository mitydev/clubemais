@php
  ...
  if ($mode === 'edit' && isset($data)) {
      $c = array_merge($defaults, (array) ($data->content ?? []));
  } else {
...
<div class="border rounded p-2">
  ...
  <div class="mb-2">
    ...
    <textarea maxlength="92"
              name="content[text]"
              rows="3"
              class="form-control">{{ $v('text') }}</textarea>
  </div>
  ...
  <div class="row g-2">
    <div class="col-6">
      ...
      <input maxlength="30"
             name="content[link_text]"
             class="form-control"
             value="{{ $v('link_text') }}">
    </div>
    <div class="col-6">
      <label class="form-label">URL do link</label>
      <input name="content[link_url]"
             class="form-control"
             value="{{ $v('link_url') }}">
    </div>
  </div>
  ...
  <div class="row g-2 mt-2">
    <div class="col-6">
      ...
      <input maxlength="40"
             name="content[kicker_title]"
             class="form-control"
             value="{{ $v('kicker_title') }}">
    </div>
    <div class="col-6">
      <label class="form-label">Chamada secundária (kicker)</label>
      <input maxlength="174"
             name="content[kicker_text]"
             class="form-control"
             value="{{ $v('kicker_text') }}">
    </div>
  </div>
